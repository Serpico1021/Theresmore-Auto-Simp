...
- **"禁止手改 base 模板"区分两种情况**（2026-08-25 用户明确澄清）：
  - **永久业务逻辑/新子系统接入点**（例如给 `getAllowedResearch()` 接入 planner）：
    默认禁止，每次都要单独向用户确认，不能援引之前的例外自行下手（Research 那次是唯一先例）。
  - **临时调试/测试代码**（例如定位一个诡异 bug 时加 `console.log` 排查数据流，验证完就删）：
    **不受"禁止手改"约束**，属于正常调试手段，不需要每次都单独发问确认——但仍要遵循
    "改完 fragments/base 后重新 build+`node --check`"的流程，且**验证完成后必须把临时调试代码
    删掉**（不能把 `console.log` 之类的排查代码遗留在生产产物里）。
